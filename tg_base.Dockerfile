...
#use --net=host to allow for localhost being forwarded, then you can also use -e to pass env. variable such as one generated by
#export MONGO_HOST=localhost
#nvidia-docker run --net=host -it -v /data:/data --name mongo_test eu.gcr.io/test-paper-doll/tg/base_all_machine_learning:1 bash
#if you did the port forwards on the host then the same ports should be ok from with the container
